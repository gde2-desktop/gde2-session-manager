...
lock_screen (GsmInhibitDialog *dialog)
{
        GError *error;
        error = NULL;
        g_spawn_command_line_async ("gde2-screensaver-command --lock", &error);
        if (error != NULL) {
...
/* copied from gde2-panel panel-util.c */
static char *
               ...
               GsmInhibitor     *inhibitor)
{
        ...
        if (desktop_filename != NULL) {
                ...
                if (desktop_file == NULL) {
                        ...
                        g_free (desktop_filename);
                        desktop_filename = g_strdup_printf ("gde2-%s.desktop", app_id);
                        error = NULL;
                        desktop_file = egg_desktop_file_new_from_dirs (desktop_filename,
                                                                       (const char **)search_dirs,
                                                                       &error);
