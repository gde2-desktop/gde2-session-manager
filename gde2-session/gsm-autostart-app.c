...
enum {
        ...
        GSM_CONDITION_GDE2          = 3,
        GSM_CONDITION_GSETTINGS     = 4,
        GSM_CONDITION_UNKNOWN       = 5
};

#define GSM_SESSION_CLIENT_DBUS_INTERFACE "org.gde2.SessionClient"
...
is_disabled (GsmApp *app)
{
        ...
        /* GSM_AUTOSTART_APP_ENABLED_KEY key, used by old gde2-session */
        ...
        if (!egg_desktop_file_can_launch (priv->desktop_file, "GDE2")) {
                g_debug ("app %s not installed or not for GDE2",
                         gsm_app_peek_id (app));
                return TRUE;
        }
                        ...
                        char      **keyp)
{
        ...
        } else if (!g_ascii_strncasecmp (condition_string, "GDE2", len)) {
                kind = GSM_CONDITION_GDE2;
...
setup_condition_monitor (GsmAutostartApp *app)
{
        ...
        if (kind == GSM_CONDITION_IF_EXISTS) {
                ...
                g_object_unref (file);
                g_free (file_path);
        } else if (kind == GSM_CONDITION_GDE2) {
...
is_conditionally_disabled (GsmApp *app)
{
        ...
        if (kind == GSM_CONDITION_IF_EXISTS) {
                ...
                disabled = g_file_test (file_path, G_FILE_TEST_EXISTS);
                g_free (file_path);
        } else if (kind == GSM_CONDITION_GDE2 && priv->condition_settings != NULL) {
                char **elems;
                elems = g_strsplit (key, " ", 2);
